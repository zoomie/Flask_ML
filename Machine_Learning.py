...
# Simple Machine learning using a Decision tree.
...
########## Building the the prediction pipeline ##########

df_test = pd.read_csv('test.csv')

#Predict_df takes in a pandas data frame and predicts if the person survived or not
def Predict_df(df_test):
    ## Preprocessing ##
    ...
    # Label encoding using the same funcitons  as the training data. #
    ...
    X_pipe = X_pipe[:,1:] # Dropping the extra col
    
    ## Calling the prediction ##
    X_pipe = sc.transform(X_pipe)
    
    x = clf.predict(X_pipe)
